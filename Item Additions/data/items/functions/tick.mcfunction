...
# KB Stick floor crafting
execute as @e[type=item,nbt={Item:{id:"minecraft:netherite_ingot",Count:1b}}] at @s as @e[type=item,nbt={Item:{id:"minecraft:stick",Count:64b}},limit=1,sort=nearest,distance=..1] at @s as @e[type=item,nbt={Item:{id:"minecraft:enchanted_book",Count:1b,tag:{StoredEnchantments:[{id:"minecraft:knockback",lvl:2s}]}}},limit=1,sort=nearest,distance=..1] at @s run function items:give_kb_stick
...
execute as @e[tag=fresh_craft_kb] at @s run particle crit ~ ~ ~ 0.5 0.5 0.5 0 5 force
